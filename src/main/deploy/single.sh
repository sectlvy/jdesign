...
MAIN_CLASS=${MAIN_CLASS:-"com.sect.ticket.platform.timer.TimerRunner"}
...
stop(){
		...
		if [ "$runningPID" ]; then
				echo "Ticket timer pid: $runningPID"
        count=0
        kwait=5
        echo "Ticket timer is stopping, please wait..."
        kill -15 $runningPID
        ...
        echo "Timer is stopped."
    		...
    		echo "Ticket timer has not been started."
    fi
}

check(){
   if exist; then
   	 echo "Ticket timer is alive."
   	 exit 0
   else
   	 echo "Ticket timer is dead."
   	 exit -1
   fi
}
